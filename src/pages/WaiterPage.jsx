...
export default function WaiterPage() {
  ...
  const playAlertSound = useCallback(async () => {
    ...
    const playNext = async () => {
      if (currentPlay < playCount && alertAudio) {
        try {
          alertAudio.currentTime = 0; // Reset to beginning
          await alertAudio.play();
          console.log(`🔊 Sound played ${currentPlay + 1}/${playCount}`);
          currentPlay++;
          if (currentPlay < playCount) {
            setTimeout(playNext, 800); // Wait 800ms between plays
          }
        } catch (error) {
  ...
  // Function to announce waiter call with voice
  const announceWaiterCall = useCallback(async (tableNumber) => {
    ...
    setTimeout(() => {
      const message = `Customer from table number ${tableNumber} is calling the waiter`;
      speakText(message);
    }, 1000); // Wait 1 second before speaking to let the sound finish

    // Repeat the announcement after 3 seconds for urgency
    setTimeout(() => {
      const message = `Attention! Table ${tableNumber} needs assistance`;
      speakText(message);
    }, 4000);
  ...
  useEffect(() => {
    const rememberedWaiter = localStorage.getItem('rememberWaiter');
    const rememberedEmail = localStorage.getItem('waiterEmail');

    if (rememberedWaiter === 'true' && rememberedEmail) {
  ...
  useEffect(() => {
    ...
    // Listen for orders that are ready to be sent to tables
    socket.on("order:readyForWaiter", (order) => {
      console.log("📥 Order ready for waiter:", order);
      setReadyOrders((prev) => {
    ...
    socket.on("waiter:called", async (callData) => {
      console.log("🔔 Waiter called from table:", callData.tableNumber);
      
      // Play alert sound and voice announcement
      try {
        await announceWaiterCall(callData.tableNumber);
      } catch (error) {
        console.error("Failed to announce waiter call:", error);
        // Fallback to just playing alert sound
        try {
          await playAlertSound();
        } catch (fallbackError) {
          console.error("Failed to play alert sound:", fallbackError);
        }
      }
      
      // Add to waiter calls list
      setWaiterCalls((prev) => {
        // Check if call already exists for this table
        const exists = prev.some(call => call.tableNumber === callData.tableNumber);
        if (!exists) {
          return [...prev, {
            ...callData,
    ...
    socket.on("order:update", (updatedOrder) => {
      console.log("📝 Order update received:", updatedOrder._id, "->", updatedOrder.status);
      setReadyOrders((prev) => {
        if (updatedOrder.status === "sent") {
          // Remove from ready orders when marked as sent
          return prev.filter(order => order._id !== updatedOrder._id);
        } else if (updatedOrder.status === "readyForWaiter") {
          // Add to ready orders if not already there
          const exists = prev.some(order => order._id === updatedOrder._id);
          if (!exists) {
            console.log("📋 Adding order to waiter queue:", updatedOrder._id);
  ...
  }, [user, alertAudio, playAlertSound, announceWaiterCall]);

  const fetchReadyOrders = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${APIBASE}/orders/ready-for-waiter`);
      if (!res.ok) {
        if (res.status === 404) {
          console.log("Ready for waiter endpoint not found, using empty state");
          setReadyOrders([]);
          setError("");
          return;
        }
        throw new Error(`Failed to fetch ready orders: ${res.status}`);
      }
  ...
  const handleLogin = async () => {
    try {
      ...
      // Check if the signed-in user is a waiter
      if (!WAITER_EMAIL.includes(result.user.email)) {
        setLoginError("You are not authorized to access the waiter page.");
        await signOut(auth);
        return;
      }

      console.log("Google login successful for waiter:", result.user.email);
    } catch (error) {
  ...
  const handleForgotPassword = async () => {
    ...
    if (!WAITER_EMAIL.includes(email)) {
      setLoginError("This email is not authorized for waiter access.");
      return;
    }
  ...
  const handleMarkAsSent = async (orderId) => {
    try {
      // Mark as sent - this will show "Sent" status to customer
      const sentRes = await fetch(`${APIBASE}/orders/${orderId}/status`, {
        method: "PUT",
        headers: {
      ...
      if (!sentRes.ok) throw new Error("Failed to mark order as sent");

      // Remove from ready orders
      setReadyOrders((prev) => prev.filter((order) => order._id !== orderId));
      
      toast.success("✅ Order sent to table successfully!", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      console.error(error);
      toast.error("❌ Failed to mark order as sent.", {
        position: "top-right",
        autoClose: 3000,
      });
    }
  ...
  const handleDismissCall = (tableNumber) => {
    setWaiterCalls((prev) => prev.filter(call => call.tableNumber !== tableNumber));
  };
  ...
  if (!user || (loginError && !authLoading)) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden transition-all duration-500 bg-gradient-to-br from-blue-50 via-white to-cyan-100">
        ...
        <div className="relative z-10 w-full max-w-md p-8 rounded-3xl shadow-2xl backdrop-blur-lg border bg-white/80 border-blue-200/50 shadow-blue-500/20 transform hover:scale-105 transition-all duration-300 hover:shadow-xl">

          <div className="flex flex-col items-center mb-8">
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center mb-4 shadow-lg relative overflow-hidden">
              <span className="text-white text-2xl font-bold">🍽️</span>
              <div className="absolute inset-0 bg-gradient-to-t from-blue-600/20 to-transparent"></div>
            </div>
            <h1 className="text-4xl font-bold mb-2 text-blue-700" style={{ fontFamily: 'ui-rounded, system-ui, sans-serif' }}>
              Cherry Myo's Waiter
            </h1>
            <h2 className="text-xl font-semibold mb-1 text-blue-700" style={{ fontFamily: 'ui-rounded, system-ui, sans-serif' }}>
              Service Portal
            </h2>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium mb-2 text-gray-700">Waiter Email</label>
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="waiter@cherrymyo.com"
                required
                className="w-full px-4 py-3 rounded-xl border bg-white border-gray-300 text-gray-700 placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  required
                  className="w-full px-4 py-3 pr-12 rounded-xl border bg-white border-gray-300 text-gray-700 placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                />
                <button
                  type="button"
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-sm hover:underline transition-colors duration-200 text-blue-600 hover:text-blue-800"
              >
                Forgot password?
              </button>
            </div>
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white font-semibold py-4 px-6 rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-lg focus:outline-none focus:ring-4 focus:ring-blue-500/50 flex items-center justify-center gap-2 text-lg"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-teal-100 relative overflow-hidden">
      {/* Floating Service Background Animation */}
      <div className="absolute inset-0 pointer-events-none">
        {[...Array(20)].map((_, i) => (
          <div
            key={i}
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl border border-blue-200/50 p-8 mb-8 transform hover:scale-[1.02] transition-all duration-300">
          <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
              ...
              <div>
                ...
                {/* <div className="flex items-center gap-2 mt-2">
                  <span className={`text-xs px-2 py-1 rounded-full ${
                    ...
                    {audioInitialized ? '🔊 Audio Ready' : '🔇 Click anywhere to enable audio'}
                  </span>
                </div> */}
              </div>
            </div>
            ...
            <div className="flex items-center gap-6 bg-white/60 backdrop-blur-sm rounded-2xl px-6 py-4 border border-blue-200">
              ...
              {/* <button
                onClick={() => announceWaiterCall("1")}
                className="px-4 py-3 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white font-semibold rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-lg focus:outline-none focus:ring-4 focus:ring-orange-500/50 flex items-center gap-2"
                title="Test voice announcement for Table 1"
              >
                <span>�️</span>
              ...
              {waiterCalls.map((call, index) => (
                ...
                >
                  ...
                  <div className="text-sm text-red-600 mb-4">
                    Called: {new Date(call.timestamp).toLocaleTimeString('en-US', {
                      hour: '2-digit',
                      minute: '2-digit',
                      hour12: true
                    })}
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleDismissCall(call.tableNumber)}
                      className="flex-1 bg-gradient-to-r from-red-600 to-pink-600 hover:from-red-700 hover:to-pink-700 text-white font-semibold py-2 px-4 rounded-xl transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2"
                    >
                      <span>🏃‍♂️</span>
                      Going to Table
                    </button>
        ...
        {loading && (
          <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg border border-blue-200 p-6 mb-6 flex items-center justify-center">
            <div className="flex items-center gap-3">
              ...
              <span className="text-blue-700 font-medium text-lg">Loading ready orders...</span>
            </div>
          </div>
        )}
          ...
          {readyOrders.length === 0 && !loading && (
            <div className="text-center py-12">
              <div className="mb-6">
                ...
                <p className="text-gray-600 text-lg">No orders ready for delivery at the moment. Great work!</p>
              </div>
            </div>
          )}
          ...
          <div className="grid gap-8 lg:grid-cols-2 xl:grid-cols-3">
            {readyOrders.map((order, index) => (
              ...
              >
                ...
                <div className="absolute top-4 right-4 text-green-400 opacity-20 text-2xl animate-pulse">
                  🍽️
                </div>
              </div>
            ))}
