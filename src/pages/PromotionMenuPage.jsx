...
export default function PromotionMenuPage() {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log("✅ Promotion Menu socket connected:", socket.id);
    });

    socket.on("menu:new", (newItem) => {
      if (newItem.promotion && newItem.promotion > 0) {
    ...
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      <main className={`p-4 min-h-screen pt-24 pb-32 transition-colors duration-300 ${
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {loading && <p className={`text-center ${darkMode ? "text-gray-300" : "text-gray-600"}`}>Loading promotion menu...</p>}
                ...
                <div className={`flex items-center mt-3 space-x-3 ${item.outofstock ? 'pointer-events-none' : ''}`}>
                  ...
                  <button
                    onClick={() => !item.outofstock && addToCart({
                      ...item,
                      price: item.promotion // Use promotional price for cart
                    })}
                    disabled={item.outofstock}
                    className={`px-3 py-1 text-white rounded transition-colors duration-200 ${
                      item.outofstock 
          ...
          <div className="sm:hidden space-y-4">
            {filteredItems.map((item) => (
              ...
              >
                ...
                {/* Out of Stock Badge */}
                {item.outofstock && (
                  <span className="absolute top-2 right-2 px-2 py-1 text-xs font-bold rounded-full bg-red-600 text-white z-10">
                    OUT OF STOCK
                  </span>
                )}
                <img
          ...
          {!loading && !error && filteredItems.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className={`text-xl font-semibold mb-2 ${
                ...
                {searchTerm ? "No promotion items found" : "No promotion items available"}
              </h3>
              <p className={`mb-6 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
                {searchTerm ? "Try searching for something else" : "Check back later for special promotions"}
              </p>
              {searchTerm && (
                <button
